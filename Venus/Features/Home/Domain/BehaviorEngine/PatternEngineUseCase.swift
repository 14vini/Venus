...
protocol PatternEngineUseCaseProtocol {
    func execute(referenceDate: Date, onAIUpdate: (@Sendable (PatternInsightsSnapshot) -> Void)?) async throws -> PatternInsightsSnapshot?
}

extension PatternEngineUseCaseProtocol {
    func execute(referenceDate: Date = Date()) async throws -> PatternInsightsSnapshot? {
        try await execute(referenceDate: referenceDate, onAIUpdate: nil)
    }
}
...
final class PatternEngineUseCase: PatternEngineUseCaseProtocol {
    ...
    init(
        ...
        venusAI: VenusAIServiceProtocol,
        ...
        snapshotAlgorithmVersion: String = "behavior-home-v6-ai"
    ) {
    ...
    func execute(
        ...
        onAIUpdate: (@Sendable (PatternInsightsSnapshot) -> Void)? = nil
    ) async throws -> PatternInsightsSnapshot? {
        ...
        async let chatSessionsTask = chatRepository.loadSessions()
        ...
        let chatSessions = (try? await chatSessionsTask) ?? []
        ...
        guard var finalSnapshot = snapshot else { return nil }

        // Store local snapshot temporarily in cache
        await snapshotCache.store(dayKey: dayKey, dataVersion: dataVersion, snapshot: finalSnapshot)
        await feedbackStore.trackSuggestion(action: finalSnapshot.nextBestAction, at: referenceDate)

        if let onAIUpdate = onAIUpdate {
            // Kick off AI generation asynchronously in the background
            Task {
                do {
                    let aiInsights = try await venusAI.generateMirrorInsights(
                        checkInHistory: moods,
                        chatSessions: chatSessions,
                        userProfile: profile
                    )
                    
                    let mergedSnapshot = PatternInsightsSnapshot(
                        nextBestAction: finalSnapshot.nextBestAction,
                        alternativeActions: finalSnapshot.alternativeActions,
                        weeklyTrend: aiInsights.weeklyTrend,
                        patternAlert: aiInsights.patternAlert,
                        weeklyInsights: aiInsights.weeklyInsights,
                        actionWhy: finalSnapshot.actionWhy,
                        proMoodForecast: finalSnapshot.proMoodForecast,
                        confidenceInsight: finalSnapshot.confidenceInsight,
                        triggerRecoveryInsight: finalSnapshot.triggerRecoveryInsight,
                        exploreActionSuggestions: finalSnapshot.exploreActionSuggestions
                    )
                    
                    // Update cache with merged snapshot
                    await snapshotCache.store(dayKey: dayKey, dataVersion: dataVersion, snapshot: mergedSnapshot)
                    
                    // Trigger UI update
                    onAIUpdate(mergedSnapshot)
                } catch {
                    print("DEBUG: Failed to generate AI mirror insights in background: \(error)")
                }
            }
        } else {
            // If no update block, do it synchronously
            do {
                let aiInsights = try await venusAI.generateMirrorInsights(
                    checkInHistory: moods,
                    chatSessions: chatSessions,
                ...
                finalSnapshot = PatternInsightsSnapshot(
                    nextBestAction: finalSnapshot.nextBestAction,
                    alternativeActions: finalSnapshot.alternativeActions,
                    weeklyTrend: aiInsights.weeklyTrend,
                    patternAlert: aiInsights.patternAlert,
                ...
                print("DEBUG: Failed to generate AI mirror insights synchronously: \(error)")
            }
        }
    ...
    private func makeDataVersion(
        ...
        digest = combineHash(digest, fingerprintChatSessions(chatSessions))
    ...
    private func fingerprintChatSessions(_ sessions: [ChatSession]) -> UInt64 {
        var digest = stableHash64("chats:\(sessions.count)")
        for session in sessions {
            let sessionRaw = "\(session.id.uuidString)|\(String(Int(session.lastMessageAt.timeIntervalSince1970)))|\(session.messages.count)"
            digest = combineHash(digest, stableHash64(sessionRaw))
        }
        return digest
    }
